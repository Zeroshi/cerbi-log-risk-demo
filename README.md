...
> This repository is a demo harness. The pipeline and Codespaces setup expect Cerbi.Scanner 1.1.0 or newer to be available as `cerbi-scanner` on `PATH`. If your Cerbi Scanner distribution uses different package IDs or supported language rules, update the install command in this repo to match your licensed scanner build.
...
- `examples/` contains sample JSON, SARIF, and Markdown summary scanner outputs for a buyer-facing walkthrough.
...
2. Wait for the container to build. The dev container uses the stable Microsoft .NET 9 devcontainer image, installs GitHub CLI support, restores both sample projects, builds the demo, and installs or updates the `Cerbi.Scanner` .NET tool.
...
Fresh rebuild expectation: after a Codespaces rebuild or after deleting and recreating the Codespace, the demo should have .NET 9, restored and built sample projects, and the current `Cerbi.Scanner` tool available as `cerbi-scanner`.

### Codespaces troubleshooting

If `scan` is not recognized, update the scanner: `dotnet tool update -g Cerbi.Scanner`. If your licensed scanner package ID differs from the public default, install that package and then rerun the scan from the repository root:
...
Both pipeline examples are intentionally copy-friendly. They scan the repository with `policies/cerbi-policy.yml`, publish JSON/SARIF/summary outputs, and fail only when the configured policy threshold is met.
